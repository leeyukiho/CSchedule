...
13. [13-学校分组能力扩展架构](./13-学校分组能力扩展架构.md)
...
- 成绩和个人资料按 target 独立缓存、独立模型、独立展示 schema；短期可共用 `FeatureCache`，但不能混成同一份业务数据。
- 学校是能力扩展边界。每个学校声明自己的 `capabilities`、`dataAccess`、字段 schema 和展示 schema，前端按 schema 渲染，不写死所有学校同一套字段。
