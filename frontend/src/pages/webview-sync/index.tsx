...
export default function WebviewSyncPage() {
  ...
  const initialBindingId = String(params.bindingId || "");
  const [activeBindingId, setActiveBindingId] = useState(initialBindingId);
  ...
  async function handlePayload(item: unknown) {
    ...
    try {
      const uploadResult = await uploadRawData(activeBindingId, {
        contextId,
        target,
      ...
      const nextBindingId = uploadResult.bindingId || activeBindingId;

      if (nextBindingId !== activeBindingId) {
        setActiveBindingId(nextBindingId);
        setStoredBindingId(nextBindingId);
      }

      const nextTargets = Array.from(new Set([...completedTargets, target]));
      setCompletedTargets(nextTargets);

      const result = await completeWebviewSync(nextBindingId, {
        contextId,
      ...
      if (result.canCloseWebview) {
        ...
        setMessage(
          `已缓存 ${nextTargets.length} 项，仍需同步：${result.missingRequiredTargets.join(", ")}`,
        );
      }
    } catch (error) {
  ...
  function finishManually() {
    ...
    void completeWebviewSync(activeBindingId, {
      contextId,
      completedTargets,
    })
      .then((result) => {
