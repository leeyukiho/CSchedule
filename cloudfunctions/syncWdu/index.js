...
function assertFrontendImportAuthorized(event) {
  ...
  const requestedTargets = normalizeProofTargets(event.targets)
  const allowedTargets = normalizeProofTargets(payload.targets)
...
function createCloudProof(event, cacheResults) {
  ...
  const proof = {
    ...
    targets: normalizeProofTargets(event.targets),
...
function normalizeProofTargets(value) {
  return [...new Set(
...
async function loginToWdu(client, config, username, password) {
  ...
  if (!publicKey.modulus || !publicKey.exponent) {
    throw new Error('WDU_LOGIN_PUBLIC_KEY_MISSING')
  }
  ...
  if (hasInvalidCredentialMessage(loginBody)) {
    throw new Error('WDU_INVALID_CREDENTIALS')
  }

  if (isLoginPage(loginBody)) {
    throw new Error('WDU_SESSION_INVALID')
  }
...
function normalizeSpecialCourse(item, index, term) {
  const rawWeeks = firstValue(item.qsjsz, item.zcd, item.sjkcgs, item.qtkcgs)

  return {
    id: String(item.jxb_id || item.kch_id || item.kch || `special-course-${index + 1}`),
    name: firstValue(item.kcmc, item.jxbmc, item.kch),
    teacher: firstValue(item.jsxm, item.xm),
    location: firstValue(item.cdmc, item.lh, item.xqmc),
    classroom: firstValue(item.cdmc, item.lh),
    campus: firstValue(item.xqmc),
    weekday: Number(item.xqj || 0),
    ...
    rawWeeks,
    remark: firstValue(item.qtkcgs, item.sjkcgs, item.kclb, item.kcxz),
    termId: term.id,
  }
}
...
async function fetchSchedule(client, config, semesterId) {
  ...
  for (const term of terms) {
    ...
    const data = parseJsonResponse(response, 'WDU_SCHEDULE_PARSE_FAILED')
    const items = Array.isArray(data.kbList) ? data.kbList : []
    const specialItems = Array.isArray(data.sjkList) ? data.sjkList : []

    courses.push(...items.map((item, index) => normalizeCourse(item, courses.length + index, term)))
    courses.push(
      ...specialItems.map((item, index) =>
        normalizeSpecialCourse(item, courses.length + index, term),
      ),
    )

    if (semesterId && (items.length > 0 || specialItems.length > 0)) {
      break
    }
  }
...
async function fetchGrades(client, config, semesterId, profileParams = {}) {
  ...
  if (!semesterId) {
    ...
    const data = parseJsonResponse(response, 'WDU_GRADES_PARSE_FAILED')
...
async function fetchExams(client, config, semesterId) {
  ...
  for (const term of terms) {
    ...
    const data = parseJsonResponse(response, 'WDU_EXAMS_PARSE_FAILED')
    const items = Array.isArray(data.items) ? data.items : []
...
function parseProfileData(profileData, homeHtml, fallbackStudentId) {
  ...
  return {
    ...
    major: normalizeMajor(firstValue(record.zymc, record.zyh_id, record.zyfxmc)),
...
async function getProfileData(context) {
  ...
  context.profileData = parseJsonResponse(response, 'WDU_PROFILE_PARSE_FAILED')
...
async function runWduProviderSync(event) {
  const targets = normalizeTargets(event)

  if (!targets.length) {
    return fail(
      'TARGET_UNSUPPORTED',
      'WDU cloud sync supports targets course, profile, score and exam only.',
      { unsupported: true },
    )
  }

  const config = WDU_CONFIG
  const client = createHttpClient(config.baseUrl)
  const homeHtml = await loginToWdu(client, config, event.username, event.password)
...
async function handleEvent(event) {
  try {
    ...
    if (event.providerId !== 'wdu') {
      return fail(
        'PROVIDER_NOT_FOUND',
        `syncWdu only supports providerId wdu, got ${event.providerId}.`,
        { unsupported: true },
      )
    }

    return await runWduProviderSync(event)
  } catch (error) {
